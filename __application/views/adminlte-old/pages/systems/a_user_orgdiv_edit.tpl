...
<script>
	...
	$.getJSON($url_module, { get_org_id: 1, parent_id: arr_filter.parent_id }, function(result){
		...
		$("#org_id").closest(".form-group").find("a").attr("href", "{$.const.PAGE_LNK}?pageid=18,129,130,131&filter=parent_id="+result.data.org_id);
	});
	...
	col.push(BSHelper.Combobox({ horz:false, label:"Division", label_link:"{$.const.PAGE_LNK}?pageid=18,129,130,131&filter=parent_id=", idname:"org_id", textField:"code_name", url:"{$.php.base_url('systems/a_orgdiv')}", remote: true, required: true }));
