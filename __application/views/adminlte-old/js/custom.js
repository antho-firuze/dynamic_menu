...
(function ($) {
  ...
  var tab_button_options = $("<li />", {"class": "active"})
      .html("<a href='#control-sidebar-options-tab' data-toggle='tab' tabindex='-1'>"
  ...
  var skin_purple_light =
      $("<li />", {style: "float:left; width: 33.33333%; padding: 5px;"})
          .append("<a href='javascript:void(0);' tabindex='-1' data-skin='skin-purple-light' style='display: block; box-shadow: 0 0 3px rgba(0,0,0,0.4)' class='clearfix full-opacity-hover'>"
          + "<div><span style='display:block; width: 20%; float: left; height: 7px;' class='bg-purple-active'></span><span class='bg-purple' style='display:block; width: 80%; float: left; height: 7px;'></span></div>"
          + "<div><span style='display:block; width: 20%; float: left; height: 20px; background: #f9fafc;'></span><span style='display:block; width: 80%; float: left; height: 20px; background: #f4f5f7;'></span></div>"
          + "</a>"
  ...
  function setup() {
    ...
    $("[data-skin]").on('click', function (e) {
      ...
      change_skin($(this).data('skin'));
			...
			$.ajax({ url: x_config_lnk,	method: "PUT",	dataType: 'json',
				data: JSON.stringify({profile:1, name:"skin", value:$(this).data('skin')})
			});
    });
    ...
    //Add the change sidebar toggle
		$("[class='sidebar-toggle']").on("click", function(){
			...
			$.ajax({ url: x_config_lnk,	method: "PUT",	dataType: 'json',
				data: JSON.stringify({profile:1, name:"sidebar", value:get($sidebar)})
			});
		});
    ...
    //Add the change timeout_list
		$("#timeout_list").change(function() {
			...
			$.ajax({ url: x_config_lnk,	method: "PUT",	dataType: 'json',
				data: JSON.stringify({profile:1, name:"screen_timeout", value:$("#timeout_list").val()})
			});
		});
